...
public class GitHubIssueDestination : IBugReportDestination
{
    ...
    internal static string FormatMarkdown(BugReport report)
    {
        ...
        if (ctx.Memory is { } mem)
        {
            var managed = FormatMb(mem.ManagedHeapBytes);
            var js = FormatMb(mem.JsHeapUsedBytes);
            sb.AppendLine($"| Heap (managed/js) | {managed} / {js} |");
        }
        ...
        // Console errors
        if (ctx.RecentConsoleErrors.Count > 0)
        {
            sb.AppendLine("### Console Errors");
            sb.AppendLine("<details>");
            sb.AppendLine($"<summary>{ctx.RecentConsoleErrors.Count} console error(s)</summary>");
            sb.AppendLine();
            sb.AppendLine("```");
            foreach (var err in ctx.RecentConsoleErrors)
            {
                var time = err.TimestampUtc.ToString("HH:mm:ss");
                sb.AppendLine($"[{time}] {err.Message}");
                if (!string.IsNullOrEmpty(err.Stack))
                    sb.AppendLine(err.Stack);
            }
            sb.AppendLine("```");
            sb.AppendLine();
            sb.AppendLine("</details>");
        ...
        // Breadcrumbs
        if (ctx.Breadcrumbs.Count > 0)
        {
            sb.AppendLine("### Breadcrumbs");
            sb.AppendLine("<details>");
            sb.AppendLine($"<summary>{ctx.Breadcrumbs.Count} breadcrumb(s)</summary>");
            sb.AppendLine();
            foreach (var crumb in ctx.Breadcrumbs)
            {
                var time = crumb.TimestampUtc.ToString("HH:mm:ss");
                sb.AppendLine($"- `{time}` **{crumb.Type}** {crumb.Detail}");
            }
            sb.AppendLine();
            sb.AppendLine("</details>");
            sb.AppendLine();
